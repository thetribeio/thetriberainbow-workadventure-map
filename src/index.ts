...
const randomNumber: string = (Math.floor(Math.random() * (0 - 100 + 1))).toString();

const handleNumberPopup = (layerName: string, popupName: string) => {
    let currentPopup: any = undefined;


    WA.room.onEnterLayer(layerName).subscribe(() => {
        currentPopup = WA.ui.openPopup(popupName, randomNumber, []);
    });
...
handleNumberPopup("PopUpZones/testPopUpLayer", "testPopUp");
